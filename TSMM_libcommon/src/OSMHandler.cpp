...
// const std::map<std::string, std::map<Road::Way::id_t, OSMWay::ptr>>& OSMHandler::getAggregatedWayInfoMap() {
//     return this->m_aggregatedWayInfoMap;
// }

// const std::map<std::string, std::map<Road::Node::id_t, OSMNode::ptr>>& OSMHandler::getAggregatedNodeInfoMap() {
//     return this->m_aggregatedNodeInfoMap;
// }
...
// void OSMHandler::addNodeObjToAggregatedNodeInfoMap(std::string layerName, Road::Node::id_t nodeId, OSMNode::ptr nodeObj) {
//     this->m_aggregatedNodeInfoMap[layerName].insert(std::pair<Road::Node::id_t, OSMNode::ptr>(nodeId, nodeObj));
// }
...
/*
 * "map"
 * this->m_allLayerNodeRef2Location
 * "obj"
 * this->m_allNodeId2NodeObj
 * this->m_allNodeId2PositionObj
 */

void OSMHandler::nodeFilter(const osmium::Node& node) {
    //    //    this->m_allLayerNodeRef2Location.insert({node.id(), node.location()});
    //
    //    // create node obj
    //    SlonlatPosition::ptr newSlonlat = std::make_shared<SlonlatPosition>(node.location().lon(), node.location().lat());
    //    //    this->m_allNodeId2PositionObj[node.id()] = std::make_shared<SlonlatPosition>(node.location().lon(), node.location().lat());
    //    //    this->m_allNodeId2PositionObj[node.id()] = SlonlatPosition(node.location().lon(),node.location().lat());
    ...
    //
    //    this->m_allNodeId2NodeObj[node.id()] = newNode;
...
/*
 * "map"
 * this->m_allLayerWayId2NodeRefDic
 * this->m_allLayerWaysStartEndRefDic
 * this->m_allLayerWayId2LengthDic
 *
 * "obj"
 * this->m_allWayId2WayObj
 * this->m_aggregatedWayInfoMap
 */
    ...
    if (way.tags().has_key(tagKey.c_str())) {
        if (remainedLayers.count(std::string(way.tags().get_value_by_key(tagKey.c_str())))) {
            ...
            if (way.tags().has_key("highway")) {
                ...
                //                this->m_wayId2Tags.insert({way.id(), tagRoadLevel});
                //                this->m_allLayerWayId2Tags.insert({way.id(), tagRoadLevel});
            }
            ...
            for (auto& refs : way.nodes()) {
                ...
                //                //                Road::SubWay::id_t localId = nodePos - 1;
                //                OSMSubWay::ptr newSubwayObj                                       = std::make_shared<OSMSubWay>(way.id(),
                //                                                                          this->m_allNodeId2NodeObj.at(fromNodeId),
                //                                                                          this->m_allNodeId2NodeObj.at(toNodeId),
                //                                                                          wayTagsMap);
                //                this->m_allSubWayId2SubWayObjs[newSubwayObj->getSubwayGlobalId()] = newSubwayObj;
                //
                //                //                this->m_allNodeId2NodeObj.at(fromNodeId)->setSubWayParentId(newSubwayObj->getSubwayGlobalId());
                //                //                this->m_allNodeId2NodeObj.at(toNodeId)->setSubWayParentId(newSubwayObj->getSubwayGlobalId());
            ...
            //            std::vector<OSMNode::ptr> nodeVectorRefs;
            //            for (auto& refs : way.nodes()) {
            //                nodeVectorRefs.push_back(this->m_allNodeId2NodeObj.at(refs.ref()));
            //            }
            ...
            //            newWay->setWayId(way.id());
            this->m_allWayId2WayObj[way.id()] = newWay; ///< for handler
            //            this->m_aggregatedWayInfoMap[std::string(way.tags().get_value_by_key(tagKey.c_str()))].insert(
            //                std::pair<Road::Way::id_t, OSMWay::ptr>(way.id(), this->m_allWayId2WayObj.at(way.id()))); ///< for handler

            //            for (const auto& nodeObj : this->m_allWayId2WayObj.at(way.id())->getNodeRefObjs()) {
            //                this->m_aggregatedNodeInfoMap[std::string(way.tags().get_value_by_key(tagKey.c_str()))].insert(
            //                    std::pair<Road::Node::id_t, OSMNode::ptr>(nodeObj->getNodeId(), nodeObj));
            //            }
        }
    }
}
...
/*
 * this->m_allLineId2LineObj
 * this->m_simplifiedNetwork
 */
...
void OSMHandler::linkWays2Lines() {
    ...
    for (const auto& layer : aggregatedWayInfoMap) {
        ...
        while (not dic.empty()) {
                ...
                for (const auto& way2Obj : dic) {
                    ...
                    if (isShare) {
                        ...
                        if (std::get<1>(angleMap) < m_parallelTolerance) {
                            ...
                            if (std::get<0>(angleMap) == 1) { // way1FrontSubWayFromNode == way2FrontSubWayFromNode
                            ...
                            } else if (std::get<0>(angleMap) == 2) { // way1FrontSubWayFromNode == way2BackSubWayToNode
                                completeWayIds.push_front(way2Obj.second->getWayId());
                                way1SubwayFrontId = way2SubwayFrontId;
                                findNextWay       = true;
                                break;

                            } else if (std::get<0>(angleMap) == 3) { // way1BackSubWayToNode == way2FrontSubWayFromNode
                                completeWayIds.push_back(way2Obj.second->getWayId());
                                way1SubwayBackId = way2SubwayBackId;
                                findNextWay      = true;
                                break;

                            } else if (std::get<0>(angleMap) == 4) { // way1BackSubWayToNode == way2BackSubWayToNode
                                completeWayIds.push_back(way2Obj.second->getWayId());
                                std::map<std::string, std::string> wayTagsMap;
                                OSMSubWay::ptr newSubway =
                                    this->createSubWay(-1,
            ...
            /// stepn: delete ways in completeWayIds;
            for (auto& visitedId : completeWayIds) {
                dic.erase(visitedId);
            }

            ///< create new Line object
            //            this->m_mergedWayDic[newWayId] = completeWayIds;
            //            newWayId += 1;
            ...
            //            this->m_allLineId2LayerName[lineObj->getLineId()] = layer.first;
...
void OSMHandler::generateSimplifiedLines() {
    ...
    for (const auto& layerLineInfo : this->m_simplifiedNetwork) {
        ...
        //        auto val = get_max(layer.second);
        //        for(const auto&it: layer.second){
        //            if(it.second->getLineLength() > val.second->getLineLength()){
        //                std::cout << layer.first << ": not the longest Line: " << it.second->getLineLength() << ", "
        //                << val.second->getLineLength() <<'\n';
        //            }
        //        }
        ...
        while (not lineId2lengthDic.empty()) {
            ...
            //
            //            this->m_aggregatedBufferedLineInfoMap[layerLineInfo.first].insert(
            //                    std::pair<Road::Line::id_t, OSMLine::ptr>(
            //                            val.first,
            //                            this->m_allLineId2LineObj.at(val.first)));
    ...
    for (auto& layerLineInfo : this->m_simplifiedNetwork) {
        for (auto& lineObj : layerLineInfo.second) {
            for (int i = 0, j = 1; j < lineObj.second->getWayObjs().size(); i++, j++) {
                ...
                //                                    this->printLineNodesRef(lineObj.second);
                //                                }
                ...
                //
                //                                std::cout << "reversing ways' number: " << count <<"\n";
                //                                if(printFlag){
    ...
    while (not lineId2lengthDic.empty()) {
        ...
        for (auto& del : toDelIds) {
            ...
            if (del != toDelIds.front()) {
                ...
                //                std::string layerName = this->m_allLineId2LayerName.at(del);
...
// const int OSMHandler::getAUsableNodeId() {
//
//     this->m_newNodeId += 1;
//     return this->m_newNodeId;
// }
...
std::map<Road::SubWay::id_t, std::vector<Road::Node::id_t>> OSMHandler::calIntersectionPoints() {
    //    std::map<std::string, std::map<Road::Line::id_t, OSMLines::ptr>> tmpData = this->m_simplifiedNetwork;
    ...
    Road::Line::id_t visitedLines;
    while (not tmpData.empty()) {
        for (const auto& line1 : tmpData) {
            visitedLines = line1.first;
...
void OSMHandler::printAllIntersection() {
    std::cout << "id;x;y"
              << "\n";
    for (const auto& it : this->m_allIntersections) {
        std::cout << std::fixed;
        std::cout << it << ";" << std::setprecision(7) << this->m_allNodeId2NodeObj.at(it)->getNodeLon() << ";" << std::setprecision(7)
                  << this->m_allNodeId2NodeObj.at(it)->getNodeLat() << "\n";
    }
}
...
void OSMHandler::cleanNetwork() {
    ...
    std::cout << "main cluster size: " << mainClusterSize << " " << ((double)mainClusterSize / (double)this->m_allIntersections.size()) * 100 << "%"
              << std::endl;
...
void OSMHandler::updateSubWayOrder(Road::Way::id_t wayId, std::vector<OSMNode::ptr>& nodeObjs) {
    ...
    for (const auto& it : nodeObjs) {
        ...
        nodePos += 1;
        //                    this->createSubWay(lineObj.second->getWayObjs().at(preWayId)->getWayId(),)
    }

    this->m_allWayId2WayObj.at(wayId)->updateSubWayObj(subwayPreObjs);
}
void OSMHandler::printSimplifiedNetwork() {
    std::cout << "lineId;wayId\n";
    for (const auto& it : this->m_simplifiedNetwork) {
        for (const auto& it2 : it.second) {
            for (const auto& it3 : it2.second->getWayObjs()) {
                //                std::cout << it2.first << ";" << it3->getWayId() <<"\n";
                if (it3->getWayId() == 923158000) {
                    std::cout << 923158000 << ": " << this->m_allWayId2WayObj.at(923158000)->getLineParentId() << "\n";
                }
                if (it3->getWayId() == 853286951) {
                    std::cout << 853286951 << ": " << this->m_allWayId2WayObj.at(853286951)->getLineParentId() << "\n";
                }
            }
        }
    }
...
std::tuple<int, double> OSMHandler::calAngleOfConnectedSubWay(Road::SubWay::id_t way1FrontSubwayId,
    ...
    if (way1FrontSubWayFromNode == way2FrontSubWayFromNode) { // 180
